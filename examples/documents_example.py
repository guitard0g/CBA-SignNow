...
if __name__ == "__main__":
    ...
    put_doc_response = signnow.Document.update(
        access_token["access_token"], doc_id["id"], doc_payload
    )
    document_data = signnow.Document.get(access_token["access_token"], doc_id["id"])
    print "The document's id:", document_data["id"]
    print "The document's name:", document_data["document_name"]
    ...
    invite_payload = {"to": to, "from": username}
    invite_response = signnow.Document.invite(
        access_token["access_token"], doc_id["id"], invite_payload
    )
    ...
    signnow.Document.download(
        access_token["access_token"], doc_id["id"], "new_file_for_me", file_path, True
    )
